...
// Obsolete, the collection "balance" is redundant, use user profile instead
// Sub-function of processNewOrder
...
// Sub-function of submitOrder
// For each book in the oder, change:
//    hold_by:    N/A
//    status:     delivered
function updateBookStatusOrdered(orderId, db){
	logger.info("ordersUtil>> updateBookStatusOrdered start...");
...
// Parameters:
// 	- orderId
exports.orderReceived = function(req, db, callback){
  logger.info("ordersUtil>> orderDelivered start...");
  ...
  collection.update(query, update, function(err, docs) {
	...
	// addBooksToBorrowerBookshelf(orderId, db);
    updateBookStatusReceived(orderId, db);
  });
}
...
//    hold_by:    borrower
//    status:     reading
function updateBookStatusReceived(orderId, db){
	logger.info("ordersUtil>> updateBookStatusReceived start...");
...
// Obsolete
// Sub-function of orderDelivered
// function addBooksToBorrowerBookshelf(orderId, db){
// 	logger.info("ordersUtil>> addBooksToBorrowerBookshelf start...");
//
// 	var collection = db.collection('orders');
// 	var query = {orderId: orderId};
// 	collection.find(query).toArray(function(err, docs){
...
// 					var status = "閱讀中";
// 					book_copies[0]["status"] = status;
// 					book_copies[0]["hold_by"] = username;
//
// 					var update = {$set: {book_copies: book_copies}};
// 					logger.info("ordersUtil>> update: " + JSON.stringify(update));
//
// 					collectionBooks.update(query2, update, function(err, docs){
// 						logger.info("ordersUtil>> 1 book received");
// 						logger.info("ordersUtil>> result: " + JSON.stringify(docs));
// 					});
// 				});
// 			}
...
// Parameters:
// 	- orderId
exports.orderClosed = function(req, db, callback){
  logger.info("ordersUtil>> orderClosed start...");
  ...
  collection.update(query, update, function(err, docs) {
	...
	// addBooksToBorrowerBookshelf(orderId, db);
    updateBookStatusClosed(orderId, db);
  });
}


// Sub-function of orderClosed
// For each book in the oder, change:
//    hold_by:    borrower
//    status:     idle
